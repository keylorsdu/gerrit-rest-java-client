...
public class AccountsRestClient extends Accounts.NotImplemented implements Accounts {
    ...
    public AccountApi create(AccountInput input) throws RestApiException {
        String url = "/accounts/" + input.username;
        JsonElement accountJson = gerritRestClient.putRequest(url, accountsParser.getAccountInputJson(input));
        AccountInfo account = accountsParser.parseAccountInfo(accountJson);
        return new AccountApiRestClient(gerritRestClient, accountsParser, account.username);
    }
